...
# Configuração de headers para requests
headers = {
...
# Escreve o cabeçalho do CSV uma vez antes de começar a coletar os dados
...
def extract_movie_details(movie_link):
    """Extrai os detalhes do filme de um link do IMDb."""
    time.sleep(random.uniform(0, 0.2))  # Pausa aleatória para evitar bloqueios
    ...
    if movie_soup:
        ...
        if page_section:
            ...
            if len(divs) > 1:
                ...
                # Salva os dados no CSV se todos os campos existirem
                if all([title, date, rating, plot_text]):
                    print(title, date, rating, plot_text)
                    save_to_csv([title, date, rating, plot_text])

def extract_movies(soup):
    """Extrai os links dos filmes e processa cada um deles."""
